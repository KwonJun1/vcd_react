...
function App() {
  ...
  const { isOpen: isViscoelasticOpen, onOpen: onViscoelasticOpen, onClose: onViscoelasticClose } = useDisclosure();
  const { isOpen: isMolecularOpen, onOpen: onMolecularOpen, onClose: onMolecularClose } = useDisclosure();

  const [isRadarChartComplete, setIsRadarChartComplete] = useState(false);
  ...
  const [otherIngredients, setOtherIngredients] = useState([
    ...
    {
      name: "오일",
      materials: ["오일 1", "오일 2", "오일 3", "오일 4", "오일 5"],
      selected: "",
      values: {},
    },
    {
  ...
  const animateChart = (finalResult) => {
    ...
    const animate = () => {
      if (frame <= totalFrames) {
        ...
        setIsRadarChartComplete(true);  // Add this line
      }
    };
  ...
  return (
    <ChakraProvider>
          ...
          <Box>
            ...
            {animatedResult && (
              <Box>
                ...
                {isRadarChartComplete && (
                  <VStack spacing={4} mt={4}>
                    <Button onClick={onViscoelasticOpen} colorScheme="blue">
                      예측 결과 상세
                    </Button>
                    <Button onClick={onMolecularOpen} colorScheme="green">
                      분자 구조 예측 결과
                    </Button>
                  </VStack>
                )}
      ...
      {/* Modal for Viscoelastic Details */}
      <Modal isOpen={isViscoelasticOpen} onClose={onViscoelasticClose} size="full">
        <ModalOverlay />
        <ModalContent>
          <ModalHeader>예측 결과 상세 — 점탄성</ModalHeader>
          <ModalCloseButton />
          <ModalBody>
          ...
          <ModalFooter>
            <Button colorScheme="blue" mr={3} onClick={onViscoelasticClose}>
              닫기
            </Button>
          </ModalFooter>
        </ModalContent>
      ...
      {/* Modal for Molecular Structure */}
      <Modal isOpen={isMolecularOpen} onClose={onMolecularClose} size="full">
        <ModalOverlay />
        <ModalContent>
          <ModalHeader>분자 구조 예측 결과</ModalHeader>
          <ModalCloseButton />
          <ModalBody>
            <Box>
              {/* <Heading as="h2" size="lg" mb={6}>
          ...
          <ModalFooter>
            <Button colorScheme="blue" mr={3} onClick={onMolecularClose}>
              닫기
            </Button>
          </ModalFooter>
        </ModalContent>
