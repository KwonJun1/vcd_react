...
function App() {
  ...
  const analyzeCompound = () => {
    ...
    setTimeout(() => {
      ...
      const scaleAndClamp = (value) => {
        const scaled = 70 + (value * 30);  // Scale to 70-100 range
        return Math.min(Math.max(scaled, 70), 100);  // Clamp between 70 and 100
      };
  
      const randomFactor = () => (Math.random() * 2 - 1) * 0.05; // Small random factor for variability
  
      const fillerEffect = (sum) => Math.sin(sum * 0.1) * 0.2; // Non-linear effect of fillers
      const oilEffect = (sum) => Math.sin(sum * 0.15) * 0.15; // Non-linear effect of oil
      const newMaterialEffect = (sum) => Math.sin(sum * 0.2) * 0.1; // Non-linear effect of new material
        // Check if there are no additional materials
      const hasNoAdditionalMaterials = 
        sum(filler) === 0 && sum(oil) === 0 && sum(newMaterial) === 0;

      // Apply penalty if there are no additional materials
      const applyPenalty = (value) => 
        hasNoAdditionalMaterials ? value * 0.95 : value; // 3% penalty

      const lrr = applyPenalty( scaleAndClamp(0.5 + 
        (nrRatio * 0.1) - (brRatio * 0.08) - (sbrRatio * 0.08) +
        fillerEffect(sum(filler)) + oilEffect(sum(oil)) + newMaterialEffect(sum(newMaterial)) +
        randomFactor() )
      );
  
      const wear = applyPenalty ( scaleAndClamp(0.5 + 
        (nrRatio * -0.05) + (brRatio * 0.1) + (sbrRatio * 0.05) +
        fillerEffect(sum(filler)) - oilEffect(sum(oil)) * 0.5 - newMaterialEffect(sum(newMaterial)) +
        randomFactor() )
      );
  
      const wet = applyPenalty ( scaleAndClamp(0.5 + 
        (nrRatio * -0.05) + (brRatio * 0.05) + (sbrRatio * 0.1) +
        fillerEffect(sum(filler)) * 0.5 - oilEffect(sum(oil)) + newMaterialEffect(sum(newMaterial)) * 0.5 +
        randomFactor() )
      );
  
      const snow = applyPenalty ( scaleAndClamp(0.5 + 
        (nrRatio * 0.05) - (brRatio * 0.05) + (sbrRatio * 0.1) +
        fillerEffect(sum(filler)) * 0.7 - oilEffect(sum(oil)) * 0.3 + newMaterialEffect(sum(newMaterial)) +
        randomFactor() )
      );
  
      const price = scaleAndClamp(0.75 - 
        (nrRatio * 0.05 + brRatio * 0.1 + sbrRatio * 0.15) -
        (sum(filler) * 0.001 + sum(oil) * 0.001 + sum(newMaterial) * 0.002) +
        randomFactor()
      );
  ...
  const animateChart = (finalResult) => {
    ...
    const totalFrames = 120;  // Increased for smoother animation
    let frame = 0;
  
    // Generate random deviations for each item
    const deviations = finalResult.data.map(() => Math.random() * 4 - 2); // Random value between -2 and 2
    ...
    const animate = () => {
      if (frame <= totalFrames) {
        ...
        const animatedData = finalResult.data.map((item, index) => {
          const baseStartValue = Math.max(item.A - 30, 60);
          const startValue = Math.max(baseStartValue + deviations[index], 60);  // Apply deviation, but ensure it's at least 60
          const range = item.A - startValue;
          
          // Create more pronounced fluctuations
          const fluctuation = (Math.sin(progress * Math.PI * 8) * range * 0.15) +
                              (Math.cos(progress * Math.PI * 12) * range * 0.1);
          
          // Combine the overall trend with fluctuations
          const animatedValue = startValue + (range * easedProgress) + fluctuation * (1 - easedProgress);
          
          return {
            ...item,
            A: Math.round(Math.min(Math.max(animatedValue, 60), item.A))
          };
        });
